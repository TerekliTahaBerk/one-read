...
export function filmEditorialReadinessChecks(
  ...
  return [
    ...
    { key: "heroImageUrl", label: "Cover image uses a valid HTTPS URL", passed: Boolean(input.heroImageUrl?.trim() && safeHttpsUrl(input.heroImageUrl)) },
    { key: "heroImageAlt", label: "Cover image has accessible alt text", passed: Boolean(input.heroImageAlt?.trim()) },
...
export function validateFilmEditorialIssue(
  ...
  const errors: Record<string, string> = {
    ...
    body: "body_too_short",
    heroImageUrl: input.heroImageUrl?.trim() ? "invalid_hero_image_url" : "hero_image_url_required",
