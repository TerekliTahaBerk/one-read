...
import { isSendDay, oneLingoSendDays } from "@/lib/schedule";
import { lingoTimezone } from "@/lib/lingo/config";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";
export const maxDuration = 300;

const ROUTE = "/api/cron/one-lingo";
...
async function handler(request: Request): Promise<Response> {
  ...
  const controls = runtimeSettings.controls.lingo;
  ...
  if (!dateParam && !isSendDay(date ?? new Date(), lingoTimezone(), oneLingoSendDays(runtimeSettings.lingoSendDays))) {
  ...
  try {
    ...
    if (!dryRun && result.subscribers.eligible > 0 && result.sends.sent === 0) {
      await notifyZeroDelivery({ productName: "OneLingo", route: ROUTE, eligible: result.subscribers.eligible });
    }
    return NextResponse.json({ ok: true, ...result });
  } catch (err) {
    const message = err instanceof Error ? err.message : "OneLingo pipeline failed";
    console.error("[cron/one-lingo] failed:", err);
    await finishRun({ id: run.id, status: "FAILED", error: message });
    await notifyRunFailure({ productName: "OneLingo", route: ROUTE, error: message });
    return NextResponse.json({ ok: false, error: "OneLingo pipeline failed" }, { status: 500 });
  }
}
