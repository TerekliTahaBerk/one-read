...
async function main() {
  ...
  if (!to) throw new Error("ADMIN_EMAIL is required for news:send-test.");

  const issue = await prisma.newsDailyIssue.findFirst({
    where: { status: "GENERATED" },
    orderBy: [{ issueDate: "desc" }, { createdAt: "desc" }],
  });
  if (!issue) throw new Error("No generated OneNews issue found. Run npm run news:generate first.");
  ...
  if (!process.argv.includes("--send")) {
    console.log("[news-send-test] dry preview only. Pass -- --send to email ADMIN_EMAIL.");
...
main()
  .catch((err) => {
    console.error("[news-send-test] failed:", err);
    process.exitCode = 1;
  })
