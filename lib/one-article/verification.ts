 ...
 *
 * What this does: proves a user controls an email by sending a 6-digit code and
 * checking it back. What this deliberately does NOT do: grant any billing/trial
 * access. Polar remains the sole source of truth for trial/paid eligibility — a
 * verified email only unlocks the ability to *edit preferences* in the public
 * flow. Codes are never stored in plaintext (HMAC only).
 */
...
export function verificationConfig() {
  return {
    ...
    /** Verified-email session lifetime — fixed at 30 minutes. */
    sessionMinutes: 30,
    /** Soft hourly request caps to limit abuse via the verification table. */
...
/**
 * HMAC of the code, bound to email + purpose so a leaked hash can't be replayed
 * in a different context. Never reversible to the plaintext code.
 */
...
/**
 * Issues a fresh verification code for email+purpose. Invalidates older
 * unconsumed codes, enforces the resend cooldown and soft hourly caps, and
 * sends the email (or logs it in development when Resend isn't configured).
 *
 * Always called the same way whether or not the email already exists — callers
 * must not branch on existence here, to avoid leaking account presence.
 */
export async function requestVerificationCode(args: {
  ...
  // Resend cooldown — based on the most recent unconsumed code.
  ...
  // Soft hourly caps (DB-based, no extra infra).
  ...
  // Invalidate older still-valid codes so only the newest is ever accepted.
  // We expire them (not consume them) so `consumedAt` stays a reliable signal of
  // a *successful* verification, and the rows remain for rate-limit counting.
  ...
  // In development without a configured email provider, surface the code to the
  // server console only so the flow stays testable. Never in production, never
  // in an HTTP response.
  const isDev = process.env.NODE_ENV !== "production";
  if (!emailSent && isDev) {
    console.log(`[one-article-verification] dev code for ${email}: ${code}`);
  }
...
/** Validates a submitted code and consumes it on success. */
...
/* ----------------------------------------------------------------------- */
/* Verified-email session (signed, httpOnly cookie)                        */
/* ----------------------------------------------------------------------- */
...
function createVerifiedSessionToken(email: string, purpose: VerificationPurpose): string {
  const cfg = verificationConfig();
...
export function setVerifiedEmailCookie(
  res: NextResponse,
  email: string,
  purpose: VerificationPurpose,
): void {
...
export function clearVerifiedEmailCookie(res: NextResponse): void {
  res.cookies.set({
    name: VERIFIED_EMAIL_COOKIE,
    value: "",
    httpOnly: true,
...
/** Reads the verified-email session from the incoming request cookie. */
export function getVerifiedEmailSession(): VerifiedEmailSession | null {
  const token = cookies().get(VERIFIED_EMAIL_COOKIE)?.value;
  return verifyVerifiedSessionToken(token);
}

/**
 * True when the current request carries a valid verified-email session for the
 * given email (case-insensitive). Used to gate preference writes.
 */
export function hasVerifiedEmail(email: string): boolean {
...
/**
 * Sends the verification email via the existing Resend infrastructure. Returns
 * true if the provider actually accepted the message, false when no provider is
 * configured (caller logs the code in development instead).
 */
...
<html lang="en">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:420px;">
            ...
            <tr>
              <td style="font-family:-apple-system,Helvetica,Arial,sans-serif;font-size:15px;line-height:1.6;color:#111111;padding-bottom:16px;">
                Your OneArticle verification code is:
              </td>
            </tr>
            <tr>
  ...
  try {
    const { messageId } = await sendDailyEmail({
      to,
      subject: "Your OneArticle code",
      text,
      html,
    });
    return Boolean(messageId);
  } catch (err) {
    console.error("[one-article-verification] send failed:", err);
    return false;
  }
}
