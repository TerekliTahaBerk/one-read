...
export function getLlmProvider(): LlmProvider | null {
  ...
  if (!provider || provider === "none" || provider === "off") {
    if (isProd && !warnedMissingProvider) {
      ...
      console.error(
        "[llm] AI_PROVIDER is not set in production. Summaries will fall back to the heuristic provider and will NOT be marked READY. Set AI_PROVIDER=openai or anthropic.",
      );
    }
    cached = null;
    return null;
